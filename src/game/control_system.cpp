...
void PlayerControlSystem::update(DeltaTime deltaTime){
    ...
    for(auto [id, bit, tc, _1]: registry.query<Transform, Player>()){
        auto right_ground = ground_right(tc.rotation);
        // move.y might not be used
        auto forward_ground = ground_forward(tc.rotation);
        ...
        for(const auto& intent: lookIntents){
            auto angle = axisAngle(intent.axis, intent.radian * dt);
            tc.rotation = angle * tc.rotation;
        }
    }
}
...
void EditorControlSystem::update(DeltaTime deltaTime){
    ...
    for(auto [id, bit, tc, _2]: registry.query<Transform, Editor>()){
        for (const auto& intent : moveIntents) {
            ...
            Vec3 world = rotate(local, tc.rotation);
            tc.position += world;
        }
        for(const auto& intent: lookIntents){
            auto angle = axisAngle(intent.axis, intent.radian * dt);
